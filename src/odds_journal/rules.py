...
class RulesetManifest(BaseModel):
    ...
    def unique_documents(self) -> "RulesetManifest":
        ...
        if self.schema_version == 1:
            ...
            if self.schema_version in {3, 4, 5, 6, 7} and any(value is None for value in contract_values):
                raise ValueError("schema_version=3/4/5/6/7 必须固定全部分析契约版本")
            calibration_values = (
            ...
            if self.schema_version in {4, 5, 6, 7}:
                ...
                allowed_contracts = {1, 2, 3} if self.schema_version == 4 else {4} if self.schema_version == 5 else {5} if self.schema_version == 6 else {6}
                ...
                expected_receipt = 6 if self.calibration_contract_version in {4, 5, 6} else 5 if self.calibration_contract_version == 3 else 4
...
def load_ruleset(root: Path, spec: str | None = None, *, allow_proposal: bool = False) -> Ruleset:
    ...
    if manifest.schema_version in {4, 5, 6, 7}:
        ...
        if manifest.calibration_contract_version in {5, 6}:
