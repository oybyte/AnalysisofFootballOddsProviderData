...
- `football-analysis@2.0.0` 是未发布的 Manifest schema 10、AnalysisReceipt V9、Knowledge Engine Contract 9 提案。其内容寻址知识 Snapshot、本地 SQLite 索引、Study/AI 旁路台账和 Contract 9 V4/V7 模型仅用于隔离验证；未发布时不得写入正式 AnalysisReceipt、Draft、Outlook、锁定、结算、复盘或统计。
...
- CLI 当前版本为 `0.12.0`，桌面工作流为 `1.13.0`，支持 Ruleset Manifest schema 1-10、AnalysisReceipt V1-V9、AnalysisOutlook V1-V6（另有未发布 Contract 9 的 V7 模型）、正式校准 Contract 1-8（另有未发布 Contract 9 parser）、Experiment Analysis Receipt V1-V4、Experiment Advisory Bundle V1/V2、MarketArchiveComparison V1、PrematchRiskWatchlist V1 和只读 PrematchReadiness V1。Knowledge Engine Contract 9 仅随 `2.0.0` proposal 以 sidecar 方式验证；默认 `agent start` 仍加载正式活动的 `1.8.0`，AI 或知识研究产物永远不能作为正式草稿输入。
...
`1.4.0` 提案将“事实与理论盘口门禁 → 分析者多市场评分矩阵 → 确定性基础排序 → profile 校准 → 候选池处置”显式化。`1.5.0` 提供 Contract 4 的草稿输入、机器评估 Bundle、AI 处置与可重建分析数据库初始实现；`1.8.0` 在此基础上以 Contract 7 强制总进球证据化与部分市场 `pass`。代码只合成已填写的离散评分，不从原始盘口值自动推导基础方向；`1.4.0` 仅可离线运行，当前 `1.8.0` 在完整赛前门禁后可进入正常锁定流程。另有未发布的 `2.0.0` Knowledge Engine 旁路提案：其完整发布预检已实现，但仍因前瞻 Outcome、人工审计和 ReleaseEvidence 未积累而保持 `shadow_ready`，不得影响正式轨。
