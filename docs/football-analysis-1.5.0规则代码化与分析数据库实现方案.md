...
后续进展（截至 2026-08-06）：`1.6.0` 已保留为 Contract 5 的历史实验快照；当前活动实验为 `1.7.0 revision 1`，通过 Contract 6 的 Intake 流水线生成隔离的预测、提示和研究产物。Analytics schema 5 已覆盖规范化事实、完整盘口趋势、来源、冲突与赛果投影；当前正式规则为 `1.8.0`。详细契约与操作见 [规则 Intake 与实验流水线](规则Intake与实验流水线.md)、[总进球证据化与部分市场 Pass](总进球证据化与部分市场Pass.md) 与 [全量数据规范化工作流](比赛全量数据规范化与趋势分析工作流.md)。
