...
后续进展（截至 2026-08-07）：`1.6.0` 已保留为 Contract 5 的历史实验快照；当前活动实验为 `1.7.0 revision 2`，通过 Contract 6 的 Intake 流水线生成隔离的预测、提示和研究产物。Analytics schema 5 已覆盖规范化事实、完整盘口趋势、来源、冲突与赛果投影；当前正式规则为 `1.8.0`。详细契约与操作见 [规则 Intake 与实验流水线](规则Intake与实验流水线.md)、[总进球证据化与部分市场 Pass](总进球证据化与部分市场Pass.md) 与 [全量数据规范化工作流](比赛全量数据规范化与趋势分析工作流.md)。

原始后续范围（历史设计）：下文未在本节或上述后续进展中列为已完成的项目均为目标设计，不能据此推定已经可用。
...
- 发布前活动版本保持 `1.3.0`；`1.5.0` 已于 2026-08-04 获得 lcz 批准，现作为不可变历史兼容基线保留。当前活动正式版本为 2026-08-06 发布的 `1.8.0`。
