...
规则回执声明校准契约时，必须使用 AnalysisOutlook V2 逐条处置八条实验规则，运行 `agent render-draft` 生成固定六章规范报告；未声明时不得提前套用校准层。草稿校验通过后、开赛前运行 `agent prepare-lock` 冻结主市场、主次方向、置信度、Outlook、规范报告、校准配置和全部赛前回执哈希，再使用候选回执完成普通锁定。禁止在开赛后生成候选回执。
