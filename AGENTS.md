...
- After repository data, rules, workflow contracts, trusted instructions, Skills, or desktop product versions change, run `agent changes`. Do not reinstall Skills for data-only or compatible ruleset changes. When it reports `workflow_breaking` (or a missing baseline), run `agent sync` directly: the manifest-authorized transaction synchronizes all four adapters, automatically certifies Codex Desktop, and commits only its generated baseline/certification artifacts. Recertify other expired products manually.
