...
8. Material that cannot become a formal fact, result or review must still appear in the target Match's 用户材料归档 block. Validate with `journal validate` and the target match validator. Report the archive path, target, per-segment application state, blockers, and confirm `generated_prediction: false`.
