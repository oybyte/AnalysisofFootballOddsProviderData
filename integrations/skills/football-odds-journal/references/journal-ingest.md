...
1. Classify intent as `store_only`, `store_and_align`, or `request_analysis`, then route one fixture: no existing record uses `journal new`; existing prematch/live material uses `journal append`; existing result or postmatch material uses `journal finish`. “完赛：”“赛果”“比赛结束” use `finish`; colloquial “复盘：” with a score or postmatch material also uses `finish`. Never turn storage-only material into a new prediction.
...
6. Call the matching high-level command with `--source-file`, `--request-file`, optional attachments and `--json`. It performs the archive, lookup, duplicate handling and permitted projection. `append`/`finish` without a unique existing target only archive to the inbox; do not create a replacement match.
7. For `journal finish`, preserve a unique full-time `H-A` score as a separate `result` segment. The CLI may derive it from a postmatch headline and attributes the source to that journal entry. Conflicting or missing scores remain blocked. `journal review` is a deprecated compatibility alias only.
