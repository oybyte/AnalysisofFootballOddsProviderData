...
3. Run `agent changes` when repository data, rules, workflow files, CLI contracts, or a desktop product version changes. Data-only changes rebuild the index; compatible rules do not reinstall this Skill. When it reports `workflow_breaking` or a missing baseline, run `agent sync` directly. It synchronizes all adapters, automatically certifies Codex Desktop, and commits only generated sync/certification artifacts; other products remain manual.
...
5. For screenshots containing odds, read `references/market-archive.md`. Transcribe only visually verified values into `MarketArchiveDraftV1`, run `journal market-archive preview --file DRAFT.yml`, and show the fixed preview. When `澳*` is red and the panel title is `详细变化`, Macau is the selected expanded provider: extract every time row in the right-hand detail panel into `macau_timeline`. Ignore the company names that remain visible in the frozen left column; they do not identify the aligned detail rows. Red means selected state, not price direction. Keep Macau trend rows separate from other handicap providers.
   For an existing fixture, run `journal market-archive compare --file CURRENT.yml`. Pass the previous visually verified task draft with `--baseline-file` when available; otherwise let the command use the latest archived capture batch. Show the full current preview, field changes, and mechanical statuses from an immutable prematch Watchlist. Never turn those statuses into a new prediction.
