...
认证必须逐产品、逐版本执行 `scenarios.yml` 当前 workflow 声明的全部场景。workflow 1.10.0 为十项，新增只读盘口增量对比和赛前风险 Watchlist 机械监测；历史 workflow 1.1.0 的五项结果、1.2.0 至 1.5.0 的六项结果、1.6.0 的七项结果、1.7.0/1.8.0 的八项结果和 1.9.0 的九项结果继续可解析。`passed` 要求对应 suite 的场景唯一且全部通过；telosWork 还要求先在产品界面导入 `.skill` 包，运行 `agent configure --product teloswork --confirm-import --imported-version VERSION`，并在结果中设置 `telos_import_confirmed: true`。
